using System.Collections;
using System.Collections.Generic;
using System.Linq;
using UnityEngine;
using System;

using System.Drawing;


[CreateAssetMenu(menuName = "Formation/CircleFormation", fileName = "Formation_Circle")]
public class Formation_Circle : Formation
{
    public override Vector3 GetPosition(int postion_index, int count, Vector3 origin)
    {
        if (_positions.Count() < count)
        {
            float radius = _size * count / (float) (2 * Math.PI);
            _positions = new Vector3[count];
            for (int i = 0; i < count; i++)
            {
                float angle = 2 * (float) Math.PI * i / count;
                float x = origin.x + radius * (float) Math.Cos(angle);
                float z = origin.z + radius * (float) Math.Sin(angle);
                _positions[i] = new Vector3(x , origin.y, z);
            }
        }
        return _positions[postion_index];
    }
}
